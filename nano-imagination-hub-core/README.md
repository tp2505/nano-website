...
| **Event** | `event` | A happening under one Initiative | `nano_date`, `nano_venue`, post content editor (long-form body below the description, optional), the standard media slot **+ Vimeo** (below), `nano_description`, `nano_gallery` (ACF Pro Gallery field: ordered attachment IDs; captions/alt on the attachment, per-video poster via the attachment nano_poster field), `nano_initiative`, `nano_related`, `nano_people` |
